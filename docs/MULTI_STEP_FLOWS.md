...
version-2 request, approval issues authorization only; the agent separately
chooses whether and when to consume it.
...
```json
{"agent_owned_flow":true,"authorization_consumed":false,"events":["build_waiting","build_authorized","publish_waiting","publish_authorized"],"execution_enabled":false,"final_state":"authorized","notifications":2,"ok":true,"separate_authorization_ids":true,"separate_request_ids":true}
```
...
decision and authorization. The agent chooses whether to consume build and
whether step 2 should exist. The bundled example leaves both authorizations in
`issued`, so no target is invoked.
...
- An agent may stop after authorization, abandon it, consume it later, or
  propose another separately gated action. Authorization expiry remains bounded.
