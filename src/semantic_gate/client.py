...
class SemanticGateClient:
    ...
    def request_action(self,action: str,*,parameters: dict,context: dict,idempotency_key: str,minimum_control: str="policy"):
        payload={"action":action,"parameters":parameters,"context":context,"idempotency_key":idempotency_key}
        if minimum_control!="policy": payload["minimum_control"]=minimum_control
        return self._call("POST","/api/v1/requests",payload)
