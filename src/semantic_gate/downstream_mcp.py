...
class StdioMCPClient:
    def __init__(self,command: Sequence[str],*,environment: Mapping[str,str] | None = None,timeout_seconds: float=5):
        if environment is not None and (not isinstance(environment,Mapping) or any(not isinstance(key,str) or not isinstance(value,str) for key,value in environment.items())): raise ValueError("MCP environment must be a string mapping")
        self.timeout_seconds=timeout_seconds; self.next_id=1; self.calls=[]; self._closed=False
        self._responses: queue.Queue=queue.Queue()
        self.process=subprocess.Popen(list(command),stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.DEVNULL,bufsize=0,env=None if environment is None else dict(environment))
