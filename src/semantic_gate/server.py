...
MCP_TOOLS = [
    ...
    {"name":"request_action","description":"Propose an action; cannot approve or execute it.","inputSchema":{"type":"object","properties":{"action":{"type":"string"},"parameters":{"type":"object"},"context":{"type":"object"},"idempotency_key":{"type":"string"}},"required":["action","parameters","context","idempotency_key"],"additionalProperties":False}},
...
class SemanticGateApplication:
    ...
    def _panel(self, session: str) -> Response:
        ...
        rows="".join(f"<tr><td>{html.escape(r['request_id'])}</td><td>{html.escape(r['action'])}</td><td>{html.escape(r['requester'])}</td><td><b>{html.escape(r['state'])}</b></td><td><button data-id='{html.escape(r['request_id'])}' data-op='approve'>Approve once</button> <button data-id='{html.escape(r['request_id'])}' data-op='deny'>Deny</button></td></tr>" for r in requests)
